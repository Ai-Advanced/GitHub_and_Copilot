...
```mermaid
graph LR
    subgraph CS["🔍 Code Scanning"]
        CS1["CodeQL"]
        CS2["자동 수정"]
        CS3["PR 통합"]
        CS4["SARIF 지원"]
    end
    subgraph SS["🔑 Secret Scanning"]
        SS1["200+ 패턴"]
        SS2["Push 보호"]
        SS3["커스텀 패턴"]
        SS4["파트너 연동"]
    end
    subgraph SC["📦 Supply Chain Security"]
        SC1["Dependabot Alerts"]
        SC2["Dependabot Updates"]
        SC3["Dependency Review"]
        SC4["SBOM 생성"]
    end
```
...
```mermaid
flowchart TD
    A["👨‍💻 개발자가 코드 작성"] --> B["🤖 Copilot이 보안<br/>베스트 프랙티스로 코드 생성"]
    B --> C["🔑 Push Protection<br/>← 시크릿 포함 시 push 차단"]
    C --> D["🔍 Code Scanning<br/>+ Copilot Autofix<br/>← PR에서 취약점 자동 탐지<br/>← AI가 수정 코드 자동 제안"]
    D --> E["📦 Dependabot<br/>← 취약한 의존성 자동 업데이트"]
    E --> F["📊 Security Overview<br/>← 조직 전체 보안 현황 대시보드"]
```
...
```mermaid
flowchart LR
    subgraph LOCAL["🖥️ 개발자 로컬 환경"]
        L1["Copilot이<br/>보안 코드<br/>생성 지원"]
        L2["pre-commit<br/>hook으로<br/>로컬 검증"]
    end
    subgraph GH["☁️ GitHub"]
        G1["① Push Protection<br/>시크릿 차단"]
        G2["② Code Scanning<br/>CodeQL 분석"]
        G3["③ Copilot Autofix<br/>AI 자동 수정 제안"]
        G4["④ Dependency Review<br/>의존성 취약점 검토"]
        G5["⑤ Dependabot<br/>자동 업데이트 PR"]
        G1 --> G2 --> G3 --> G4 --> G5
    end
    subgraph OPS["📊 운영 환경"]
        O1["⑥ Security<br/>Overview<br/>대시보드"]
        O2["조직 전체<br/>보안 현황<br/>모니터링"]
    end
    LOCAL -->|"push"| GH
    GH -->|"데이터 집계"| OPS
```
...
```mermaid
flowchart LR
    A["📄 소스코드"] --> B["🗄️ CodeQL DB 생성"] --> C["🔍 쿼리 실행"] --> D["📊 결과 반환"]
```

**지원 언어:** JavaScript, TypeScript, Python, Java, C/C++, C#, Go, Ruby, Swift, Kotlin
...
```mermaid
flowchart TD
    A["🔍 CodeQL이 취약점 발견"] --> B["🤖 Copilot Autofix"]
    B --> B1["1. 취약점 컨텍스트 분석"]
    B1 --> B2["2. 데이터 흐름 추적"]
    B2 --> B3["3. 수정 코드 생성"]
    B3 --> B4["4. 자연어 설명 첨부"]
    B4 --> C["📋 PR에 수정 제안 표시"]
    C --> D["⚠️ SQL Injection (CWE-89)<br/><br/>Before:<br/>query = SELECT * FROM users<br/>WHERE name = ' + name<br/><br/>🤖 Copilot Autofix 제안:<br/>query = SELECT * FROM users<br/>WHERE name = ?<br/>params = [name]<br/><br/>💡 파라미터화된 쿼리로 방지"]
    D --> E["✅ 수정 적용 | ❌ 무시 | ✏️ 편집"]
```
...
```mermaid
graph LR
    subgraph DETECT["🔍 탐지 계층"]
        D1["200+ 파트너 패턴<br/>(AWS, Azure, GCP,<br/>Slack, Stripe 등)"]
        D2["커스텀 정규식<br/>패턴 정의 가능"]
        D3["커밋 히스토리<br/>전체 스캔"]
    end
    subgraph PROTECT["🛡️ 보호 계층"]
        P1["Push Protection<br/>커밋 push 시점에서<br/>시크릿 포함 코드 차단"]
        P2["바이패스 정책<br/>- 허용: 감사 로그 기록<br/>- 차단: 완전 차단 (권장)<br/>- 관리자 승인: 승인 후 허용"]
        P3["알림 & 대응<br/>- 파트너사 자동 통보<br/>- 시크릿 자동 폐기 (일부)<br/>- Slack/Teams 알림 연동"]
    end
```
...
```mermaid
graph LR
    subgraph AL["⚠️ Alerts (알림)"]
        A1["취약 의존성<br/>알림 발생"]
        A2["수동 확인"]
    end
    subgraph SU["🔐 Security Updates<br/>(보안 업뎃)"]
        S1["보안 패치"]
        S2["PR 자동 생성"]
        S3["자동화 가능"]
    end
    subgraph VU["📦 Version Updates<br/>(버전 업뎃)"]
        V1["최신 버전"]
        V2["PR 자동 생성"]
        V3["자동화 가능"]
    end
...
```mermaid
graph TB
    subgraph SO["📊 Security Overview"]
        subgraph OV["📊 Overview (개요)"]
            OV1["전체 알림 수: Critical 3 / High 12 / Medium 45 / Low 89"]
            OV2["트렌드: 지난 30일 알림 추이 그래프"]
            OV3["Top 10 위험 리포지토리 순위"]
        end
        subgraph COV["🔍 Coverage (커버리지)"]
            COV1["Code Scanning 활성화: 85/100 repos (85%)"]
            COV2["Secret Scanning 활성화: 92/100 repos (92%)"]
            COV3["Push Protection 활성화: 78/100 repos (78%)"]
            COV4["Dependabot 활성화: 90/100 repos (90%)"]
        end
        subgraph REM["🔧 Remediation (수정 현황)"]
            REM1["평균 수정 시간: 4.2일"]
            REM2["미해결 Critical 알림: 3건"]
            REM3["이번 달 해결된 알림: 28건"]
            REM4["Autofix 적용률: 62%"]
        end
        subgraph PRE["🛡️ Prevention (예방)"]
            PRE1["Push Protection 차단 횟수: 156건/월"]
            PRE2["PR에서 차단된 취약점: 89건/월"]
            PRE3["바이패스 요청: 12건 (승인 8, 거부 4)"]
        end
    end
```
