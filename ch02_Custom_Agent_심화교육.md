# 📌 Chapter 2. Custom Agent 심화 교육

> **소요 시간:** 25~30분 (데모 + 실습 포함)  
> **학습 목표:** Copilot 커스터마이징 기초(Instructions, MCP)부터 멀티 에이전트 오케스트레이션, 현업 활용까지 단계적으로 학습합니다.  
> **사전 준비:** VS Code 최신 버전, GitHub Copilot 확장 설치, `.github/agents/` 폴더 생성  
> **이전 단계:** [Chapter 1 — Copilot 소개](./ch01_GitHub_Copilot_소개.md) | **다음 단계:** [Chapter 3 — GHAS 보안 심화](./ch03_GHE_GHAS_보안심화.md)
...
| 순서 | 주제 | 시간 |
|------|------|------|
| 3.1 | Copilot 커스터마이징 단계 | 2분 |
| 3.2 | Custom Instructions (커스텀 인스트럭션) | 3분 |
| 3.3 | MCP (Model Context Protocol) 연동 | 2분 |
| 3.4 | Custom Agent 아키텍처 이해 | 5분 |
| 3.5 | Sub-Agent & 오케스트레이션 | 5분 |
| 3.6 | 🔧 실습: 멀티 에이전트 워크플로우 구축 | 8분 |
| 3.7 | 현업 활용 시나리오 & 실습 | 8분 |
| 3.8 | 에이전트 프롬프트 꿀팁 | 4분 |
...
```mermaid
graph TB
    subgraph hierarchy["Copilot 커스터마이징 계층"]
        L1["Level 1: Custom Instructions<br/>.github/copilot-instructions.md<br/>프로젝트 전체에 자동 적용되는 규칙/스타일"]
        L2["Level 2: Custom Agents<br/>.github/agents/*.agent.md<br/>전용 도구와 역할을 가진 AI 페르소나"]
        L3["Level 3: Agent Skills<br/>.github/skills/<br/>스크립트/리소스 포함, 이식 가능한 복합 능력"]
        L4["Level 4: MCP (Model Context Protocol)<br/>외부 시스템(DB, API, Slack 등) 연동"]
    end
    L1 --> L2 --> L3 --> L4
```
...
```mermaid
graph LR
    A["🗂️ Planner<br/>(계획)<br/><br/>tools:<br/>- search<br/>- web/fetch<br/><br/>읽기 전용 ✅"] -->|"Handoff<br/>구현 시작"| B["⚙️ Implementer<br/>(구현)<br/><br/>tools:<br/>- edit<br/>- terminal<br/><br/>편집 가능 ✅"]
    B -->|"Handoff<br/>리뷰 시작"| C["🔍 Reviewer<br/>(리뷰)<br/><br/>tools:<br/>- search<br/>- web/fetch<br/><br/>읽기 전용 ✅"]
```
...
```mermaid
graph TB
    FB["🎯 Feature Builder<br/>(오케스트레이터)<br/><br/>tools: agent<br/>agents: Researcher, Implementer"]
    FB --> R["🔍 Researcher<br/>(조사 전문)<br/><br/>tools:<br/>- search/codebase<br/>- web/fetch<br/>- search/usages<br/><br/>읽기만 가능 ✅"]
    FB --> I["⚙️ Implementer<br/>(구현 전문)<br/><br/>tools:<br/>- edit<br/>- terminal<br/><br/>user-invocable: false<br/>↑ 직접 호출 불가,<br/>오케스트레이터만 호출"]
```
