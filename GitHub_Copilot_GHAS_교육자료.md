...
Level 2: Reusable Prompts       → 반복 작업 자동화
Level 3: Custom Agents          → 전문가 AI 페르소나 생성
Level 4: Agent Skills           → 스크립트 포함 이식 가능한 능력
Level 5: MCP (Model Context Protocol) → 외부 도구 연동
```
...
## 3.3 Reusable Prompts (재사용 가능한 프롬프트)

`.github/prompts/` 폴더에 `.prompt.md` 파일로 정의합니다.
...
---
다음 요구사항으로 REST API 엔드포인트를 생성해주세요:

1. Express.js 라우터 사용
2. 입력값 유효성 검사 (Joi/Zod)
3. 에러 핸들링 미들웨어 적용
4. Swagger/OpenAPI 문서 주석 포함
5. 해당 엔드포인트의 단위 테스트 포함

사용자에게 API 이름과 HTTP 메서드를 #tool:vscode/askQuestions 로 질문하세요.
```

> VS Code에서 `/create-api-endpoint` 슬래시 커맨드로 호출 가능!
>
> ⚠️ **참고:** `${input:variableName}` 문법도 사용 가능하지만, LLM이 패턴을 인식하여 질문하는 방식이므로 공식 권장은 `vscode/askQuestions` 도구입니다.
>
> 📎 **공식 문서:** https://code.visualstudio.com/docs/copilot/customization/prompt-files

## 3.4 MCP (Model Context Protocol) 연동
...
└── prompts/
    └── create-api-endpoint.prompt.md # 재사용 프롬프트
