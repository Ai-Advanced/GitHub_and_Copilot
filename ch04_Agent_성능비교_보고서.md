...
sequenceDiagram
    ...
    participant RV as 🛡️ Reviewer
    participant IM as ⚙️ Implementer

    FB->>PL: ① 계획 수립<br/>"현대화 계획을 ADR로 세워줘"
    PL-->>FB: ADR + C4 반환

    FB->>RS: ② 기술 조사 (계획서 전달)<br/>"인증/DB/검증 기술 비교 조사해줘"
    RS-->>FB: Technology Radar 반환

    FB->>RV: ③ 보안 감사 (계획서+기술조사 전달)<br/>"OWASP/CWE/CVSS 기준으로 전수 감사해줘"
    RV-->>FB: 13개 취약점 + CVSS + 공격시나리오

    FB->>IM: ④ 구현 (계획서+기술선택+취약점 전달)<br/>"모든 분석 결과를 반영하여 전체 재작성해줘"
    IM-->>FB: 12개 모듈 현대화 코드 반환
...
```mermaid
graph TB
    subgraph lifecycle["프로젝트 라이프사이클"]
        기획 --> 설계 --> 구현 --> 리뷰 --> 배포 --> 운영
    end
    설계 --- PL["📋 Planner<br/>계획 수립"]
    설계 --- RS["🔍 Researcher<br/>기술 조사"]
    구현 --- IM["⚙️ Implementer<br/>구현"]
    리뷰 --- RV["🛡️ Reviewer<br/>코드 리뷰"]
    리뷰 --- PR["📝 PR Reviewer"]
    배포 --- MO["🔄 Modernizer<br/>리팩터"]
    운영 --- OB["📖 Onboarding Guide"]
    기획 -.- FB["🎯 Feature Builder (전체 오케스트레이션)"]
```
